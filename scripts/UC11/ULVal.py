...
class ULVal:
    ...
    def __init__(self, dmname, cfg, ucf):
        self.ucf = ucf
    ...
    def visitULVal(self, stguclass, pos, refnum):
        eprint(f"NEEDYIELDvULV {dumper.dumps(self)} {pos} {refnum}")
        siz = self.sizeOrNone()
        if siz is None: die(f"NEEDED SIZ BY NOW: {self}")
        if self.isprim:
            yield URefInfo.URefInfo(stguclass,refnum,pos,siz,self.uc,self.uvname) # just the prim
        else:
            yield from self.uc.visitClassPlus(pos, refnum+1)          # a whole instance of the class
